...
export class StudentConsultReportByClass extends BaseReport{
    html = ''
   ...
   private addReport(header: HeaderReport, dataList: DataRowModel[],sumarizeList:DataRowModel) {
    ...
    getReportHeader(header: HeaderReport) {
        return `<div class="a4">
        <p class="header left">หน้ารายงานผล (ระดับชั้น)</p>
        <p class="header center">การให้คำปรึกษา</p>
        <p class="header center">ชั้นมัธยมศึกษาปีที่<span>${this.getClassOnly(header.className)}</span> ภาคเรียนที่ <span>${header.term}</span> ปีการศึกษา <span>${header.year}</span></p>`
    }
