...
export class StressReportByClass extends BaseReport{
    html = ''
   ...
   private addReport(header: HeaderReport, dataList: DataRowModel[],sumarizeList:DataRowModel[]) {
        ...
        report += this.getReportTableSumarize(sumarizeList,header)
        return report
    
    }
    getReportTableSumarize(sumarizeList: DataRowModel[],header: HeaderReport) {
        let paper =`<div class="a4">
        <div class="content">
            <p class="title">ผลประเมินโรคซึมเศร้า ชั้นมัธยมศึกษาปีที่<span> ${this.getClassOnly(header.className)} </span> </p>
           <table class="mt">
            <tr>
